...
const getUsersById = async (id) => {
  if (route.query.id) {
    ...
    const res = await fetch(`${import.meta.env.VITE_APP_TITLE}/api/users/${id}`, {
      headers: {
...
const deleteUser = async () => {
  if (confirm('Are you sure you want to delete ?') == true) {
    const res = await fetch(`${import.meta.env.VITE_APP_TITLE}/api/users/${route.query.id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    <div class="card-body p-10">
      ...
      <div v-if="isedit == false">
        ...
        <div>
          ...
          <button
            class="bg-gray-600 w-[50%] hover:bg-gray-700 rounded-lg text-white font-bold py-2 px-8 border-grey-700 hover:border-grey-800"
            @click="goBack"
          >
            CLOSE
          </button>
     ...
     -->
      <div v-if="isedit == true">
        ...
        <div>
          <button
            class="bg-red-700 w-[100%] hover:bg-gray-400 rounded-lg text-white font-bold py-2 px-10 border-gray-700 hover:border-gray-500 mt-1"
            @click="goBack"
          >
            CANCEL
          </button>
